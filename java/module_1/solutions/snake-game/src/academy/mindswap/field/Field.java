...
public final class Field {
    ...
    public static void clearSnake(Snake snake) {
        Terminal.Color snakeColor = null;
        Position head = snake.getHead();
        for (Position p : snake.getFullSnake()) {
            if (!p.equals(head)) {
    ...
    public static void clearFruit(Fruit fruit) {
        screen.putString(fruit.getPosition().getCol(), fruit.getPosition().getRow(), " ", null, null);
    }

    public static void drawScore( int score) {
        screen.putString(20, 0, "SCORE: "+score, Terminal.Color.GREEN , null);
    }

    public static void drawMaxScore( int maxScore) {
        screen.putString(32, 0, "MAX SCORE: "+maxScore, Terminal.Color.RED   , null);
    }

    public static void gameOver(Terminal.Color color,boolean remove ) {
       ...
       String[] over=game.split("\n");
        for (int i = 0; i < over.length; i++) {
            if(remove){
                over[i]="                                          ";
            }
            screen.putString(33, 5+i, over[i], color, null);
        }
